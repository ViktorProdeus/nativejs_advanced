...
const Lesson3 = () => {
    ...
    const [searchResult, setSearchResult] = useState<ResultItemType[] | string>([]);
    const [searchNameByType, setSearchNameByType] = useState<string>('');
    const [searchResultByType, setSearchResultByType] = useState<ResultItemType[] | string>([]);
    const [searchNameSeason, setSearchNameSeason] = useState<string>('');
    const [searchSeasonsResult, setSearchSeasonsResult] = useState<ResultItemType[] | string>([]);
    ...
    const searchFilm = async () => {
        try {
            ...
            const {Response, Search, Error} = data;
            Response === 'True' ? setSearchResult(Search) : setSearchResult(Error);
        } catch (err) {
            console.log('err ', err);
        } finally {
            setSearchName('');
    ...
    const blocks = [
        {
            ...
            actionName: setSearchName,
            actionSearch: searchFilm,
            response: searchResult,
            moreButtons: false,
            firstBtnName: 'movie',
            secondBtnName: 'series',
        },
        {
            name: "Search by type",
            valueName: searchNameByType,
            actionName: setSearchNameByType,
            actionSearch: searchByType,
            response: searchResultByType,
            moreButtons: true,
            firstBtnName: 'movie',
            secondBtnName: 'series',
        }
                    ...
                    <div className={'search-block'}>
                        ...
                        >
                            <InputBase
                                sx={{ml: 1, flex: 1}}
                                placeholder={b.name}
                                inputProps={{'aria-label': b.name}}
                                value={b.valueName}
                                onChange={(e) => b.actionName(e.currentTarget.value)}
                            />
                            <SearchIcon style={{opacity: 0.3}} />

                            <Divider sx={{height: 28, m: 0.5}} orientation="vertical" />
                            {!b.moreButtons && <IconButton color="primary" sx={{p: '10px'}} aria-label="directions"
                                         onClick={b.actionSearch}>
                                <DirectionsIcon />
                            </IconButton>}

                            {b.moreButtons &&
                                <>
                                <Button onClick={b.actionSearch} data-t={b.firstBtnName}>{b.firstBtnName}</Button>
                                <Divider sx={{height: 28, m: 0.5}} orientation="vertical" />
                                <Button onClick={b.actionSearch} data-t={b.secondBtnName}>{b.secondBtnName}</Button>
                            </>
                            }
                        </Paper>
                    </div>
